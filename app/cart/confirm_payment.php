<?php
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
       ...
       if(trim($paymentMethod) == "" || empty($paymentMethod)){
           $_SESSION["error"] = "Payment Method cannot empty";
    ...
    try{
        ...
        foreach($carts as $cart){
         ...
         if(!$stmt->execute()){
          ...
          //validate there is not record return from Update query above,
          //meaning no matching product with enought quantity exist
          if($stmt->rowCount() <= 0){
